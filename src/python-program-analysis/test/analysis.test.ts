...
import { DataflowAnalyzer, IDataflow, IRef, ReferenceType, RefSet, SymbolType } from '../data-flow';
import { Set } from '../set';
...
describe('getDefs', () => {
    ...
    function getDefsFromStatements(specs?: JsonSpecs, ...codeLines: string[]): IRef[] {
        let code = codeLines.concat('').join('\n');
    ...
    describe('detects definitions', () => {
        ...
        describe('that are weak (marked as updates)', () => {
            it('for dictionary assignments', () => {
                let defs = getDefsFromStatement("d['a'] = 1");
            ...
            it('for property assignments', () => {
                let defs = getDefsFromStatement('obj.a = 1');
                expect(defs.length).to.equal(1);
                expect(defs[0].level).to.equal(ReferenceType.UPDATE);
                expect(defs[0].name).to.equal('obj');
            });

            it('for property chains', () => {
                let defs = getDefsFromStatement('obj.a.b.c = 1');
                expect(defs.length).to.equal(1);
                expect(defs[0].level).to.equal(ReferenceType.UPDATE);
                expect(defs[0].name).to.equal('obj');
            });
            ...
            it('for mixed chains', () => {
                let defs = getDefsFromStatement('obj["x"].a = 1');
                expect(defs.length).to.equal(1);
                expect(defs[0].level).to.equal(ReferenceType.UPDATE);
                expect(defs[0].name).to.equal('obj');
            });

            it('for calls on properties', () => {
                const specs = { __builtins__: {} }; // m not included, so assume it has side effect
                let defs = getDefsFromStatements(specs, 'obj.a.m()');
                expect(defs.length).to.equal(1);
                expect(defs[0].level).to.equal(ReferenceType.UPDATE);
                expect(defs[0].name).to.equal('obj');
            });
        ...
        describe('; given a spec,', () => {
            ...
            it('assumes method call affects the receiver element, without a spec', () => {
                const specs = { __builtins__: {} };
                let defs = getDefsFromStatements(specs, 'x=C()', 'x["foo"].m()');
                expect(defs).to.exist;
                expect(defs.length).to.equal(2);
                expect(defs[0].name).to.equal('x');
                expect(defs[0].level).to.equal(ReferenceType.DEFINITION);
            ...
            it('assumes method call affects the receiver field, without a spec', () => {
                const specs = { __builtins__: {} };
                let defs = getDefsFromStatements(specs, 'x=C()', 'x.f.m()');
                expect(defs).to.exist;
                expect(defs.length).to.equal(2);
                expect(defs[0].name).to.equal('x');
                expect(defs[0].level).to.equal(ReferenceType.DEFINITION);
