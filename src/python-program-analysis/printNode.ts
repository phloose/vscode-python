import { IArgument, IParameter, SyntaxNode } from './python-parser';
...
function printTabbed(node: SyntaxNode, tabLevel: number): string {
    ...
    switch (node.type) {
        ...
        case 'dict':
            const s: string[] = node.entries.map(e => `${e.k}:${e.v}`);
            return `{${s}}`;
        case 'dot':
        ...
        case 'from':
            return (
                ...
                ' import ' +
                node.imports
                    .map(im => im.path + (im.alias ? ' as ' + im.alias : ''))
                    .join(comma)
            );
        case 'global':
        ...
        case 'import':
            return (
                ...
                'import ' +
                node.names
                    .map(n => n.path + (n.alias ? ' as ' + n.alias : ''))
                    .join(comma)
            );
        case 'index':
...
function printParam(param: IParameter): string {
    return (
        (param.star ? '*' : '') +
        (param.starstar ? '**' : '') +
        param.name +
