 ...
 */
export class CellExecution<TCell extends IAnalysisCell> {
    readonly cell: TCell;
            ...
            for (let cellSlice of slicedExecution.cellSlices) {
                ...
                if (!cellSlices[cell.executionEventId]) {
                    let newCellSlice = new CellSlice(
                        { cell: cell.deepCopy(), slice: new LocationSet(), executionTime: cellSlice.executionTime });
...
export type CellExecutionCallback<TCell extends IAnalysisCell> = (exec: CellExecution<TCell>) => void;

/**
 * Makes slice on a log of executed cells.
 */
export class ExecutionLogSlicer<TCell extends IAnalysisCell> {
            ...
            .map(execution => {
                ...
                let cellSlices = cellOrder.map(
                    sliceCell => new CellSlice(
                        { cell: sliceCell, slice: cellSliceLocations[sliceCell.executionEventId], executionTime: cellExecutionTimes[sliceCell.executionEventId] }));
     ...
     */
    public getDependentCells(executionEventId: string): IAnalysisCell[] {
        ...
        return new Set<IAnalysisCell>(c => c.persistentId,
