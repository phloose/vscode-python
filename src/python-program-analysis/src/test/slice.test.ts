...
describe('slice', () => {
  ...
  it("bug 108622", () => {
    const code = [
      ...
      /*3*/ 'data = [["Alex", 10, "male", "S"], ["Bob", 12, "male"], ["Clarkette", 13, "female"]]',
      /*4*/ 'df = pd.DataFrame(data, columns=["Name", "Age", "Sex", "Embarked"])',
      /*5*/ 'df["Embarked"].fillna("S" , inplace = True)',
      /*6*/ 'df["Embarked"] = df["Embarked"].map({"nan":0,"S":1,"C":2,"Q":3}).astype(np.int)',
      /*7*/ 'df.info()'
    ];

    const tree = parse(code.join('\n'));
    const loc = new LocationSet({ first_line: 7, first_column: 0, last_line: 7, last_column: 10 });
    let locations = slice(tree, loc);
    const lineNums = locations.items.map(loc => loc.first_line);
