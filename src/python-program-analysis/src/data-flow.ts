...
export class DataflowAnalyzer {
    ...
    private getDelDefs(del: ast.Delete) {
        let targetsDefListener = new DelTargetsDefListener(del, this._symbolTable);
        return targetsDefListener.defs;
    }

    private getFromImportDefs(from: ast.From) {
...
function findRoot(node: ast.SyntaxNode): ast.Name | undefined {
    switch (node.type) {
...
class DelTargetsDefListener extends AnalysisWalker {
    constructor(del: ast.Delete, symbolTable: SymbolTable) {
        super(del, symbolTable);
        if (del.targets) {
            for (let target of del.targets) {
